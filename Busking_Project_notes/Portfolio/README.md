...
> 이번 버스킹 현황 게시판 프로젝트는 기획부터 배포까지 전 주기를 경험한 첫 팀 프로젝트였습니다.
> 초기엔 설계 없이 기능 구현부터 들어가 Git 충돌·동시 수정 등 협업 난관을 겪었고, ERD·API 명세서 재작성과 대규모 리팩터링이 불가피했습니다.
>
> 문제를 해결하기 위해 기능 개발을 잠시 멈추고 기초 설계 → Git 협업 플로우 → CI 자동 검증을 단계적으로 정비한 결과, 개발 속도와 코드 품질이 모두 향상되었습니다.
>
> 이 경험을 통해 **기능 구현 역량뿐 아니라 개발 순서·Git/GitHub·CI/CD·테스트 코드까지 아우르는 종합적인 개발 프로세스 이해가 필수**라는 점을 절실히 깨달았습니다.
> 앞으로는 초기 설계와 협업 환경을 먼저 단단히 구축한 뒤 개발에 착수하는 습관을 유지하겠습니다.
