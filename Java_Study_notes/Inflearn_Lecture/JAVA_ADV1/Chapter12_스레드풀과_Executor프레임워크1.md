# 📚 Chapter 12: 스레드 풀과 Executor 프레임워크 1
...
5. V get()
    ...
    - 예외
      - InterruptedException: 대기 중에 현재 스레드가 인터럽트된 경우에 발생.
...
1. invokeAll()
   - <T> List<Future<T>> invokeAll(Collections <? extends Callable<T> tasks) throws InterruptException
